...
export function convert(compatibleFieldSet: any): AbstractFormControlConfig {
    const { controlType, renderData, renderComponent, controls } = compatibleFieldSet

    if (controlType === 'control') {
        const comp = getControlRenderComponent(renderComponent)
        return FormConfig.Control('', { renderData, renderComponent: comp }, compatibleFieldSet.validators)
    }
    ...
    if (controlType === 'group') {
        const subcontrols = controls.reduce((acc: any, controlJSON: any) => {
            ...
            acc[name as unknown as string] = convert(controlData)
            return acc
        }, {})
        const comp = getGroupRenderComponent(renderComponent)
...
function getGroupRenderComponent(type: 'rowGroup' | 'columnGroup'): any {
    switch (type) {
        case 'rowGroup':
            return RowGroup
        case 'columnGroup':
...
function getControlRenderComponent(type: 'input' | 'textarea' | 'select'): (props: any) => JSX.Element {
    switch (type) {
        case 'input':
            return BasicInput
        case 'textarea':
